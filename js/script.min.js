    ...
    // Плюс-минус в карточке товара
        ...
        (() => {
            ...
            if (event.target.classList.contains('amount-select-minus')) {
                ...
                let parent = findParent(event.target, 'product-item-add-cart'), // event.target.parentElement
                    valueInput = parent.querySelector('.amount-select-value')

                    if (valueInput.value <= 1) return false
                    valueInput.value = +valueInput.value - 1

                    if (parent.classList.contains('added-to-cart-w')) {
   ...
   document.querySelectorAll('.product-card').forEach(item => {
    ...
    ;(function(){
        ...
        if (productInCart (btn.getAttribute('data-id'))) {
            ...
            document.querySelector('.amount-select-value').setAttribute("value", productInCart (btn.getAttribute('data-id')).amount, event.target.getAttribute('data-price'))
        }
...
if (document.querySelector('.cart-wrapper')) {
    ...
    (() => {
        ...
        let order = JSON.parse(window.localStorage.getItem('order')),
            ...
            let response = fetch('/php/cart_data.php', {
                method: 'POST',
                headers: {  
            ...
            document.body.addEventListener('click', () => {
                ...
                if (event.target.classList.contains('cart-amount-plus')) {
                    ...
                    products.length ? window.localStorage.setItem('order', JSON.stringify(make_array (products))) : window.localStorage.removeItem('order')
...
function returnCartValues () {
    let data = JSON.parse(window.localStorage.getItem('order')) || []
    ...
    document.querySelector('.cart-price-value').innerText = summ
}
