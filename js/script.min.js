        ...
        let order = JSON.parse(localStorage.getItem('order')),
                ...
                setTimeout(() => {
                    ...
                    document.querySelector('.cart-wrapper').insertAdjacentHTML('beforeend', `
                        <div class = "cart-item cart-item-top">
                            ...
                            <div class = "cart-weight item-title">Вес/объем</div>
                            <div class = "cart-amount item-title">Количество (шт)</div>
                    ...
                    products.forEach(item => {
                        ...
                        document.querySelector('.cart-wrapper').insertAdjacentHTML('beforeend', `
                            <div class = "cart-item cart-item-row" data-id = "${item.id}">
                                <div class = "cart-pic">
                                    <a href = "/catalog/${item.url_name}" target = "_blank">
                                        <img src="/images/catalog/brands/${item.id}/1_s.png" alt="" class = "product-cart-pic">
                                    </a>
                                </div>
                                <div class = "cart-title simple-text"><a href = "/catalog/${item.url_name}" target = "_blank" class = "thin-font-style">
                                    <span>${item.category}</span> <span class = "bold-text-600">${item.brand} ${item.name} ${[window.screen.width <= 1050 ? ` (${item.weight})` : ``]}</span></a>
                                </div>

                                ${[window.screen.width > 1050 ? ` <div class = "cart-weight simple-text">${item.weight}</div>` : ``]}
                                ...
                                <div class = "cart-price simple-text">
                                    <p class = "cart-value">
                                        ${item.sale > 0 ? `<span class = "cart-price-sale">${getFinalPriceDecorated (item.price, 0)} р.</span>` : ''}
                                        <span class = "cart-price-fullprice" data-fullprice = ${getFinalPrice (item.price, item.sale)}>
                                        ${getFinalPriceDecorated (item.price, item.sale)} р.</span>
                                    </p>
                                </div>
                                <div class = "cart-summ simple-text">${(item.amount * getFinalPrice (item.price, item.sale)).toLocaleString()} р.</div>
                    ...
                    document.querySelector('.cart-wrapper').insertAdjacentHTML('beforeend', `
                        <div class = "cart-item-final">
                            <div class = "cart-item-info">Итого: <span class = "final-price" id = "final-price">${countFinalSumm(products).toLocaleString()} р.</span></div>
                        </div>
            ...
            document.body.addEventListener('click', () => {
                ...
                if (event.target.classList.contains('cart-amount-minus')) {
                    ...
                    row.querySelector('.cart-summ').innerText = parseInt(span.innerText) * parseInt(row.querySelector('.cart-price-fullprice').getAttribute('data-fullprice')) + ' р.'
                    document.getElementById('final-price').innerText = countFinalSumm(products) + ' р.'
                }
            })
            ...
            document.addEventListener('click', () => {
                ...
                (() => {
                    ...
                    if (!event.target.classList.contains('cart-close')) return false
...
function returnCorrectWord (value) {
    ...
    value.slice(-1) == 1 ? value = ' товар' : ''
    value.slice(-1) == 0 || value.slice(-1) >= 5 && value.slice(-1) <= 20 ? value = ' товаров' : ''
    value.slice(-1) >= 2 && value.slice(-4) <= 20 ? value = ' товара' : ''
...
// Функция, которая считает стоимость со скидкой или без с округлением в пользу клиента единым числом без пделения на разряды
